...
vec4 Fog(vec4 diffuse, vec3 worldPos, float nightDay, bool nether, bool end, float rain, float darkness, vec3 fog, float renderDistance){
    ...
    float width = 1.0 - rcWidth + 0.001;
      ...
      le = min(1.0, le);
    #else
      le = max(0.0, ((le/(renderDistance*reFac)) - width)) + addWidth;
      le = min(1.0, le);
      if(end){
