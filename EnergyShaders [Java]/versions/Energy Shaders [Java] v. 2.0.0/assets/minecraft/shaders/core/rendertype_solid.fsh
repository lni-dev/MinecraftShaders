...
void main() {
    ...
    WorldInfo worldInfo;
    worldInfo.colorRaw = texture(ES_TEXTURE_ATLAS, ES_UV_TEXTURE_ATLAS) * vertexColor * ColorModulator;
    worldInfo.normal = normal.xyz;
    worldInfo.screenPos = inScreenPos.xyz;
    worldInfo.playerCenteredPos = inWorldPos.xyz;

    worldInfo.fogColor = FogColor;
    worldInfo.fogStart = FogStart;
    worldInfo.fogEnd = FogEnd;
    worldInfo.fogShape = FogShape;
    ...
    if(worldInfo.nether) {
        ...
        ESRenderFogNether(color, worldInfo);

    } else if(worldInfo.end) {
        ESRenderEnd(color, worldInfo);
        ESRenderFogEnd(color, worldInfo);
    } else {
