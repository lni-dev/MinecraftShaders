...
//Settings for the Shadows
  //static const bool Shadow = true;
    static const float shwE = 0.880; //The width of the Shadows... If you set it to 0.0 there will also be Sunlight in a cave :P
    static const float2 fSwE = float2(0.015, 1.0 / 0.015); //import: if you change the first, you must also change the third number! The 1.0 always(!) stays the same... but if you want you can also try to change it xD. If yoj want to know what that does: that the width of the fade in!
    //static const bool Shadow_fade_in = true;
  
  static const bool Player_Shadow = true;
    //static const bool Player_Shadow_fade_in = true;
  
//for the water :D
  //static const bool cloud_reflection = false; //not working on Win10!
  static const float4 watercolor = float4(0.0,0.0,0.9,0.3); //the water color near you
  static const float4 farwatercolor = float4(0.1, 0.7, 0.4, 0.9); //the water color in the distance...
...
if(FOG_COLOR.r < 0.25 && FOG_COLOR.b<0.25 && ((FOG_CONTROL.x < 0.55 && FOG_CONTROL.x > 0.1)) && FOG_COLOR.r > 0.1 && FOG_COLOR.r > FOG_COLOR.b && PSInput.uv1.y == 0.0){
  nether = 1.0;
}
...
#ifdef NEAR_WATER
	if(uw == 0.0){
		diffuse = lerp(watercolor*(1.0 - itsnight.g*0.4), farwatercolor*(1.0 - itsnight.g*0.4), max(0.0, min(1.0,  PSInput.color.a-itsnight.g*0.6)));
