...
//Ambient.h:
			...
			//N normal vector
			float4 Ambient(float4 diffuse, float shadow, float light, float nightDay, float rain, float darkness, float3 N){
				...
				float3 sunColor = lerp(SUN_LIGHT_COLOR_DAY, SUN_LIGHT_COLOR_NIGHT, nightDay);
					...
					float b;
						...
						diffuse.rgb = lerp(diffuse.rgb, float3(r, g, b), nightDay);
				...
				#ifdef SHADOW
					diffuse.rgb = lerp(diffuse.rgb, diffuse.rgb * shadowColor, shadow);
				#endif
...
//Fog.h
			...
			float4 Fog(float4 diffuse, float3 worldPos, float nightDay, float rain, float darkness){
					...
					float3 espeFogColor = lerp(ESPE_FOG_COLOR_DAY, ESPE_FOG_COLOR_NIGHT, nightDay);
...
//Water.h:
			...
			float4 water(float3 waterColor, float3 pos, float3 camPos, float3 N){
				...
				transparency = (1.0 - abs(transparency)) + 0.4;
...
void main(in PS_Input PSInput, out PS_Output PSOutput)
{
	...
	float4 mTime = float4(sky_color.r, abs(sky_color.r - 0.5), getRain(), min(1.0, (1.0 - sky_color.r)*1.2));	//DAY-NIGHT, SUNSET, RAIN, NIGHT-DAY
...
diffuse = Ambient(diffuse, shadow, PSInput.uv1.x/*light*/, mTime.w/*nightDay*/, mTime.z/*rain*/, mCave/*darkness*/, float3(0.0, 0.0, 0.0) /*N*/);
...
if(b.r <= 0.01 && b.g <= 0.01 && b.b <= 0.01 && b.a > 0.5){
	...
	diffuse = water(waterColor, PSInput.wPos, CAMERA_OFFSET, float3(0.0, 1.0, 0.0) /*N*/);
}
...
//diffuse.rgb = TEXTURE_0.Sample(TextureSampler0, PSInput.uv0).rgb;
