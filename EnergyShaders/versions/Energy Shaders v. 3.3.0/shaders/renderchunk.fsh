...
vec4 waterColor(vec4 Reflection, float a, vec3 waterColor, float YO, float sh){
	vec4 wC = vec4(mix(waterColor, Reflection.rgb, min(1.0, max(0.0, a + Reflection.a - 0.35-YO)) - (1.0 - sh*1.2)), a);
	return wC;
}

float circleCosX(POS3 pos){
	return cos(pos.x) * cos(pos.z*0.5);
}

float circleCosZ(POS3 pos){
	return cos(pos.x*0.5) * cos(pos.x);
}

float circleSinX(POS3 pos){
	return sin(pos.x) * sin(pos.z*0.5);
}

float circleSinZ(POS3 pos){
	return sin(pos.x*0.5) * sin(pos.x);
}
...
highp float fact = 0.00065;
	...
	vec4 dxp = texture2D( TEXTURE_0, vec2(uv0.x+0.1 * fact, uv0.y) );
	vec4 dxm = texture2D( TEXTURE_0, vec2(uv0.x-0.1 * fact, uv0.y) );
	vec4 dyp = texture2D( TEXTURE_0, vec2(uv0.x, uv0.y+0.2 * fact) );
	vec4 dym = texture2D( TEXTURE_0, vec2(uv0.x, uv0.y-0.2 * fact) );

#else
	vec4 diffuse = texture2D_AA(TEXTURE_0, uv0 );
	vec4 dxp = texture2D_AA( TEXTURE_0, vec2(uv0.x+0.0008, uv0.y) );
	vec4 dxm = texture2D_AA( TEXTURE_0, vec2(uv0.x-0.0008, uv0.y) );
	vec4 dyp = texture2D_AA( TEXTURE_0, vec2(uv0.x, uv0.y+0.8) );
	vec4 dym = texture2D_AA( TEXTURE_0, vec2(uv0.x, uv0.y-0.0008) );

#endif

vec4 d2 = mix(dxp, dxm, 0.5);
vec4 d3 = mix(dyp, dym, 0.5);
vec4 d4 = mix(dxp, dyp, 0.5);
vec4 d5 = mix(dxm, dym, 0.5);

vec4 d6 = mix(mix(d2, d3, 0.5), mix(d4, d5, 0.5), 0.5);
vec4 d7 = mix(mix(d4, d2, 0.5), mix(d3, d5, 0.5), 0.5);

//diffuse.rgb = diffuse.rgb * (1.0 - abs(diffuse.rgb - d5.rgb)*max(0.0, 3.5 /getColorDifference(diffuse.rgb, d5.rgb)));

if(color.g <= color.r){
	diffuse.rgb = mix(diffuse.rgb, vec3(diffuse.rgb * 0.5),max(0.0, min(1.0, getColorDifference(diffuse.rgb, d5.rgb)*3.0)) );
}
//diffuse = mix(diffuse, mix(d2, d3, sin(uv0.x*90.0) / sin(uv0.y*90.0)), sin((uv0.x-uv0.y)*90.0));
...
#ifdef E_WATER
	if(water){
		diffuse = waterColor(AllReflections(vec3(0.0, 1.75, 0.0), fragPos.xyz, SKY_HEIGH, E_SKY_COLOR, 0.0), diffuse.a, WATER_COLOR, 0.0, uv1.y);
