...
/*vec3 getReflection(vec3 pos, vec3 reflectionVector){
				vec3 cloudPos = worldPosToCloudPos(reflectionVector, SKY_HEIGHT);

				vec4 sky = calcSky(cloudPos);


				return sky.rgb;
			}*/

			vec4 water(vec3 waterColor, vec3 pos, vec3 camPos, vec3 N){
