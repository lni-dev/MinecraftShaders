...
float wave2(float2 p, float f1, float f2, float t){
	return sin(p.x * f1 + t) * sin(p.y * f2 + t)*0.5;
};

float3 noise(float3 p){
	
	float a = wave2(p.xz, 0.1, 0.2, TIME*0.5);
	float c = wave2(p.xz, 0.2, 0.07, TIME);
	
	float b = wave2(p.xz + float2(a,a), 1.0, 1.0, TIME*0.7);
	float r = wave2(p.xz + float2(c,c), 1.0, 0.75, -TIME*1.2);
	float g = wave2(p.xz + float2(a,c), 1.1, 0.9, TIME*1.5);
...
if(water){
	...
	float3 n = noise(fixPos.xyz)*2.0;
	refrac = Enoise(fixPos.xyz + float3(TIME, TIME, TIME)) + Enoise(fixPos.xyz*-1 + float3(TIME, TIME, TIME)) - Enoise(fixPos.xyz*2.0 + float3(TIME, TIME, TIME));
	
	float3 waterC = float3(0.1, 0.2, 1.9 + refrac*0.4);
