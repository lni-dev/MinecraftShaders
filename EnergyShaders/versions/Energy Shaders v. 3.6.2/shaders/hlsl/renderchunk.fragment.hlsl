...
//Settings.h:
			...
			#define TORCH_COLOR_CAVE float3(0.6, 0.25, 0.05)
			#define SUN_LIGHT
			...
			#define CAMERA_OFFSET float3(0.0, 0.0, 0.0)
      ...
      #define CLOUD_FREQUENCY 1.0
			...
			#define WATER_COLOR_AT_NIGHT_PER_CENT float4(0.25, 0.25, 0.35, 0.65)
//Settings.h-end
...
//Ambient.h:
			...
			float4 Ambient(float4 diffuse, float3 screenPosition, float shadow, float light, float nightDay, float rain, float darkness, float3 N){
				...
				float3 sunColor = lerp(SUN_LIGHT_COLOR_DAY, SUN_LIGHT_COLOR_NIGHT, nightDay);
				 ...
				 light *= TORCH_LIGHT_STRENGTH;
					...
					float b;
						...
						caveDif = lerp(caveDif * 0.15 * CAVE_TONE_MAPPING_COLOR_FACT, nRGB, clamp((1.0 - darkness), CAVE_MAX_DARKNESS, 1.0));
				...
				#ifdef SHADOW
					diffuse.rgb = lerp(diffuse.rgb, diffuse.rgb * shadowColor, clamp(shadow - (light), 0.0, 1.0));
				#endif
