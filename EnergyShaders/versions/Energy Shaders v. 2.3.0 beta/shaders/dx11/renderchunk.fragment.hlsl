...
float4 AllReflections(min16float3 CamPos, min16float3 p, float sky_height, float3 sky_color, float YO){
	...
	AllR = lerp(HSky_color, cloudReflection.rgb, max(0.0, min(1.0, cloudReflection.a*0.2)));
	
	
	return float4(AllR, cloudReflection.a);
};

float4 waterColor(float4 Reflection, float a, float3 waterColor, float YO){
	float4 wC = float4(lerp(waterColor, Reflection.rgb, min(1.0, max(0.0, a + Reflection.a - 0.35-YO))/* - (1.0 - PSInput.uv1.y*1.2)*/), a);
	return wC;
};
...
if(water){
	...
	float3 waterC = float3(0.1, 0.2, 0.9);
	
	float3 fromPToPos = float3(0.0, 1.75, 0.0) - posE;
	float XZl = length(posE.xyz);
	
	float transparenzy = clamp((XZl/RENDER_DISTANCE)*6.5, 0.5, 1.0)*1.5;
	
	diffuse = waterColor(AllReflections(float3(0.0, 1.75, 0.0), posE.xyz, 200.0,  float3(0.1, 0.35, 0.7), refrac*5.0), transparenzy, waterC, refrac*0.0);
